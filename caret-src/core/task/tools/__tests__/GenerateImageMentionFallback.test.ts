// CARET MODIFICATION: Ensure mention image attachments can be reused as tool references.
...
describe("GenerateImageToolHandler mention fallback", () => {
	...
	it("uses the latest mention attachment set when scoped images are missing", async () => {
		const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "caret-mention-fallback-"))
		try {
			HostProvider.reset()
