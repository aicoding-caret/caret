...
- `api.ts`의 `*Models` 객체가 `} as const` 또는 `} as const satisfies Record<...>`로 끝나는지 확인
...
## 참고 사항
- 스크립트는 `api.ts`를 파싱하여 데이터를 추출하므로, `api.ts`의 형식이 변경되면 스크립트도 수정 필요
...
- 이 파일 수정 시 `.agents/skills/update-model-list/SKILL.md`도 동일하게 업데이트
