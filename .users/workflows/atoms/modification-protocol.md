...
## 핵심 원칙
**CARET MODIFICATION 주석을 사용하여 Cline 원본 파일 수정 최소화**
...
## 검증 단계
- [ ] CARET MODIFICATION 주석이 명확하게 존재
- [ ] 수정이 최소 (최대 1-3줄)
- [ ] 컴파일 성공
- [ ] 익스텐션이 오류 없이 로드
...
## 대안적 접근
1. **먼저 시도**: caret-src/ 래퍼 사용 (Level 1)
2. **두번째 시도**: CARET 주석과 함께 최소 수정 (Level 2)
3. **최후 수단**: 완전한 문서화와 함께 대규모 수정 (Level 3)

## 일반 가이드라인
이 프로토콜은 업스트림 변경을 병합하는 능력을 유지하면서 Cline 파일의 안전한 수정을 보장합니다. CARET MODIFICATION 주석 접근 방식은 deprecated된 .cline 백업 방법을 대체합니다.
