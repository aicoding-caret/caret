<# 
// CARET MODIFICATION: Windows PowerShell build+run script for Caret CLI.
#>
[CmdletBinding()]
param(
	[AllowNull()]
...
$LogPath = if ($LogPath) { $LogPath } else { Join-Path $buildRoot ("logs\\caret-build-run-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss")) }
