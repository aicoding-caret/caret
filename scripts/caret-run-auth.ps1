<# 
// CARET MODIFICATION: Windows PowerShell auth run script for Caret CLI (no rebuild).
#>
[CmdletBinding()]
param(
	[AllowNull()]
...
if (-not $env:CARET_SKIP_KILL) {
	...
	Write-Host "[info] CARET_SKIP_KILL set; skipping host/core shutdown before auth run"
}
