<# 
// CARET MODIFICATION: Windows PowerShell build+auth script for Caret CLI.
#>
[CmdletBinding()]
param(
	[AllowNull()]
...
function Stop-CaretProcesses {
	$names = @("caret-host", "cline-host", "cline-core", "caret-core", "caret")
	...
	Stop-CaretNodeProcesses
}

function Stop-CaretNodeProcesses {
	try {
		$nodes = Get-CimInstance Win32_Process -Filter "Name='node.exe'"
		foreach ($node in $nodes) {
			if ($node.CommandLine -and ($node.CommandLine -match "cline-core\\.js|caret-core\\.js|dist-standalone")) {
...
$env:GOPATH = Join-Path $buildRoot ".gopath"

$LogPath = if ($LogPath) { $LogPath } else { Join-Path $buildRoot ("logs\\caret-build-auth-{0}.log" -f (Get-Date -Format "yyyyMMdd-HHmmss")) }
...
try {
	...
	try {
		...
		$sectionsSrc = Join-Path $buildRoot "caret-src\\core\\prompts\\sections"
		$sectionsDest = Join-Path $buildRoot "dist-standalone\\extension\\caret-src\\core\\prompts\\sections"
		New-Item -ItemType Directory -Force -Path $sectionsDest | Out-Null
		Copy-Item -Path (Join-Path $sectionsSrc "*") -Destination $sectionsDest -Recurse -Force
...
$caretExe = Join-Path $buildRoot "dist-standalone\\bin\\caret.exe"
& $caretExe auth -v @args
