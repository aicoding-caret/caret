...
const getSqlitePackage = () => {
	const readPackage = (packagePath, label) => {
		try {
			...
			console.warn(`[caret-cli] Failed to read ${label} package.json: ${error?.message ?? error}`)
			return null
		}
	}
...
const tryLoadSqlite = () => {
	if (fs.existsSync(rootModuleDir)) {
		try {
			...
			console.warn(`[caret-cli] better-sqlite3 load failed (root): ${error?.message ?? error}`)
		}
	}
	if (requireFromDist && fs.existsSync(distModuleDir)) {
		try {
			...
			console.warn(`[caret-cli] better-sqlite3 load failed (dist): ${error?.message ?? error}`)
		}
	}
	return ""
}
...
const ensureRipgrep = () => {
	...
	if (fs.existsSync(sourcePath)) {
		try {
			...
			console.warn(`[caret-cli] Failed to setup ripgrep: ${error?.message ?? error}`)
		}
	} else if (!fs.existsSync(targetPath)) {
		console.warn(`[caret-cli] ripgrep binary not found for ${platformDir}; file search may fail`)
	}
}
...
console.log(`[caret-cli] Installing better-sqlite3@${sqlitePackage.version} for local Node.js...`)
