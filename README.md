This repository has moved to a new location due to trademark issues. 본 저장소는 상표권 이슈로 인해 아래의 새로운 주소로 이동했습니다.

New Repository: https://github.com/caretive-ai/careti

Multi-language Announcements
🇰🇷 한국어 (Korean) 죄송합니다. 본 저장소는 상표권 이슈로 인해 브랜드 변경을 진행하였으며, 위의 링크(Careti)로 이동했습니다. 모든 최신 업데이트는 새 저장소에서 확인해 주세요.

🇺🇸 English (English) Sorry. This repository has moved to the link above due to trademark legal issues and a subsequent rebranding to Careti. Please follow the new repository for future updates.

🇯🇵 日本語 (Japanese) 申し訳ありません。本リポジトリは商標権の問題によりブランド名を変更し、上記の新しいリポジトリ（Careti）に移動いたしました。今後の更新は移行先をご確認ください。

🇨🇳 中文 (Chinese Simplified) 抱歉，由于商标版权问题，本仓库已更名为 Careti 并迁移至上方链接。请前往新仓库获取后续更新。

🇫🇷 Français (French) Désolé. Ce dépôt a été déplacé vers le lien ci-dessus en raison de problèmes juridiques liés à la marque et d'un changement de nom pour Careti.

🇪🇸 Español (Spanish) Lo sentimos. Este repositorio se ha trasladado al enlace superior debido a problemas legales de marca registrada y al cambio de nombre a Careti.

🇩🇪 Deutsch (German) Entschuldigung. Dieses Repository wurde aufgrund von markenrechtlichen Problemen und der Umbenennung in Careti unter den oben genannten Link verschoben.


* 본 레파지토리는 이동 당시의 복사본으로 링크의 소실을 우려하여 한시적으로 데이터를 유지합니다.
