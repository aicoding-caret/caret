...
> **注意**: Caret v0.4.0 基于 Cline v3.38.2。

### 🎉 Cline v3.38.2 上游合并
- **合并提交**: `8723b386f` (合并分支: `main_backup_20251128202033`)

### 新增
- **Cline v3.38.2 完全整合**: 完全整合了 Cline v3.38.2 的所有功能，包括最新的模型支持和核心改进（包括 Claude Opus 4.5）。
- **双账户系统**: 在 Caret 模式（扩展功能）和 Cline 模式（原生体验）之间无缝切换。
- **Caret CLI (测试版)**: 推出了全新的统一 CLI 工具 `caret`，支持两种模式和增强的身份验证。包含 LiteLLM 支持。
- **增强的提供商设置**: 为 LiteLLM 和 BizRouter 提供模型自动获取和实时健康检查功能。
- **JSON 提示系统**: 支持通过 JSON 文件进行动态系统提示配置。
- **输入历史**: 提供类似终端的输入历史导航功能，并具有持久化存储。
- **快捷键**: 添加了用于取消 (Esc) 和恢复 (Ctrl+Shift+R) 任务的快捷键。

### 修复
- **终端挂起**: 修复了在 Linux 系统上使用 shell 集成时终端命令挂起的问题。
- **品牌**: 在所有 UI 组件和 CLI 输出中正确恢复了 Caret 品牌。

## [3.38.2]

### 新增
- 新增 Claude Opus 4.5（Anthropic / OpenRouter / Bedrock / Vertex），已同步提示缓存价格
...
- **新模型支持**:
  ...
  - 增强的模型信息和价格显示
- **新功能**:
  - `.caretignore` 文件支持（兼容.clineignore）以排除Caret的文件访问
