# Codex 오픈소스 대비 Caret Smart Terminal 개선·통합 계획

작성: Codex 에이전트  
목표: Smart Terminal을 “상위 에이전트 허브”로 완성하고, Codex 오픈소스에서 배울 점을 Caret에 반영해 멀티 에이전트/멀티 CLI 제어 역량을 강화한다.

---

## 1) 현재 Caret Smart Terminal 상태 (코드 기준)
- **핵심 경로**: `caret-src/integrations/terminal/interactive/`  
...
[Headless/External Entry Points]
  ├─ caret-cli (신규 하위명령: terminal, exec-like)
...
1. **서비스 경계 명확화**  
   - 새 파일: `caret-src/integrations/terminal/interactive/service/TerminalService.ts` (팩토리 + export)  
...
2. **IntelliJ 어댑터 스텁**  
   - 새 파일: `caret-src/integrations/terminal/interactive/adapters/intellij/IntelliJTerminalAdapter.ts`  
   - 내용: `ITerminalAdapter` 구현 스텁 + `TODO(pt y4j wiring)` + 테스트 skip guard.  
   - 목적: 코드 구조를 두 IDE가 공유하도록 정리하고, 빌드 시 선택 가능하도록 준비.
3. **스트림 브리지 초안**  
   - 새 파일: `caret-src/integrations/terminal/interactive/stream/TerminalEventBus.ts`  
...
3. **System prompt/tool metadata**  
   - `caret-src/core/prompts/sections/TERMINAL_TOOL_GUIDE.json`: 스트리밍/헤드리스 사용법, 새로운 액션(resize/signal) 설명 추가.  

### Phase 3 – Headless 진입점(멀티 에이전트 허브)
1. **CLI 엔드포인트**  
   - 새 파일: `cli/terminal-gateway.ts`  
   - 명령: `pnpm caret terminal --action open/send/read/list --json` 형태로 JSONL 이벤트 출력.  
   - 내부에서 `TerminalService` 사용, `--session-id`로 기존 세션 재사용.  
   - 안전 가드: Git 리포 체크, `--sandbox` 옵션은 기존 CLI 정책 준수.
2. **JSONL 이벤트 포맷 정리**  
   - 새 문서: `caret-docs/development/terminal-events.md`  
   - 스키마: `{type, session_id, data, ts}` 형식으로 Codex 이벤트(`item.*`)와 호환되게 설계.

### Phase 4 – 테스트/검증
1. **단위 테스트**  
   - `caret-src/integrations/terminal/interactive/__tests__/unit/`에 EventBus/Service 테스트 추가.  
...
2. IDE별 설정 선택지 문서화 (`caret-docs/development/frontend-backend-interaction-patterns.md`에 추가).
...
- **문서 위치**:  
  - 본 보고서: `caret-docs/보고서(reports)/프로젝트 개선/`  
  - 이벤트 스펙: `caret-docs/development/terminal-events.md` (신규)  
  - 시스템 프롬프트 가이드: `caret-src/core/prompts/sections/TERMINAL_TOOL_GUIDE.json` 업데이트  
...
- **기본 전략**: 새로운 기능은 `caret-src/` 아래에 추가·주입하고, `src/`(Cline 경로)는 최소 수정만 수행. Tool 등록/enum 변경 같은 필수 수정은 `// CARET MODIFICATION:` 주석을 남겨 upstream 머지 시 식별 가능하게 유지.  
...
- **태그/베이스라인 관리**: Squash 머지 후 `caret-squash-baseline-*` 태그를 남겨 `--onto` 재베이스 전략을 계속 사용할 수 있게 한다(문맥: `caret-docs/guides/upstream-merging.md`).  
...
1. **단위 테스트 (caret-src/integrations/terminal/interactive/__tests__/unit/)**  
...
2. **통합 테스트 (caret-src/integrations/terminal/interactive/__tests__/integration/)**  
...
5. **문서/가이드 검증**  
   - `caret-docs/development/terminal-events.md`(신규)와 `TERMINAL_TOOL_GUIDE.json` 업데이트가 코드와 일치하는지 검토.  
...
#### CLI/헤드리스(JSONL) 시나리오
- 명령: `pnpm caret terminal --action open --command "node" --json`  
  - 기대 JSONL: `{"type":"session_opened","session_id":"...","data":{"command":"node"}}`
- 명령: `pnpm caret terminal --action send --session-id <ID> --input "1+2"`  
  - 기대 JSONL: `{"type":"command_sent",...}` 이어서 `{"type":"output","data":"3\n"}`
- 명령: `pnpm caret terminal --action read --session-id <ID>`  
  - 기대 JSONL: `{"type":"output","data": "..."}`
- 명령: `pnpm caret terminal --action close --session-id <ID>`  
...
#### 문서 스냅샷
- `caret-docs/development/terminal-events.md`와 실제 이벤트 필드 비교: 스키마 `type/session_id/data/ts` 일치 여부.  
...
- 멀티 에이전트(가위바위보) 시나리오:  
  ...
  - 기대 흐름:  
    ...
    4) Caret이 두 선택을 비교해 승/패/무승부 판단 후 요약 출력(예: `codex: 보, claude-code: 가위 → claude-code 승`).  
  ...
  - 멀티 에이전트 토론+중재 시나리오(유저가 흐름을 볼 수 있어야 함):  
    ...
    - 기대 흐름:  
      1) `open` 두 번으로 세션 준비.  
      2) 라운드별로:  
       - Caret이 각 세션에 `send`로 질문/반론 전달(예: 라운드1: 장점, 라운드2: 단점, 라운드3: 결론 요청).  
       - 각 `read` 결과를 즉시 사용자에게 중간 로그로 보여줌(예: `Round1 codex: ...`, `Round1 claude: ...`).  
       - Caret이 중재 코멘트/요약을 사용자에게 출력.  
    3) 3라운드 후 Caret이 두 답변을 종합해 최종 결론 메시지를 사용자에게 제시.  
      4) 세션 정리(`close` 두 번), `list` 0 확인.  
    - 주의:  
      - `terminal_stream=true`이면 JSONL 이벤트를 그대로 표면화해 사용자에게 스트림으로 보여줄 수 있음.  
      - `terminal_stream=false`라면 각 `read` 결과를 Caret이 수집해 요약/중재 메시지로 출력.  
      - 라운드 수, 주제는 사용자 프롬프트로 바꿔서 재사용 가능.

---

## 9) 스트림 노출 및 멀티-뷰 UX 방향
- **우선권**: 가능한 한 `terminal_stream=true`를 켜서 JSONL 이벤트를 **그대로 사용자에게 흘려보내고**, Caret은 중재/요약만 추가로 출력한다(이중 노출).  
...
- 우리 Smart Terminal Hub는 **저층 실행/스트림은 Cline CLI/TerminalManager를 재사용**하고, Caret 고유 가치를 **세션 중재·멀티 에이전트·요약/판정 UX**에 집중하도록 범위 조정.  
- 별도 CLI 게이트웨이를 추가할 경우, 네임스페이스를 `caret-src/cli/**` 등으로 분리하고 기본값 off(환경변수/플래그)로 두어 머지 충돌을 최소화.  
