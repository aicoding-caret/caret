...
**개발 방법**: Caret TDD 원칙 준수 (Phase 0 RED → Phase 1 GREEN → Phase 2 REFACTOR)
...
# 6. Caret 계정 시스템 확인
grep -r "glm" caret-src/ --include="*.ts" | grep -i model
```
...
```typescript
// caret-src/__tests__/major-models-integration.test.ts (신규)
...
**Phase 0: RED**
- [ ] 테스트 파일 생성: `caret-src/__tests__/major-models-integration.test.ts`
- [ ] 20개 테스트 케이스 작성
- [ ] 테스트 실행 (모두 실패 예상)
...
// 하지만 .agents/context/caret-rules.json이나 yaml 같은 파일을 AI가 읽는 문제 발생
```
...
.agents/
├── context/
│   ├── caret-rules.json        # AI 시스템 규칙 (시스템용)
...
# 3. 현재 caret-rules.json 사용처 확인
grep -r "caret-rules\.json\|caret-rules\.yaml" src/ --include="*.ts"
```
...
```typescript
// caret-src/__tests__/init-context-separation.test.ts (신규)
...
export class ContextSeparator {
  ...
  static loadSystemContext(): SystemContext {
    const rulesPath = '.agents/context/caret-rules.json'
...
**Phase 0: RED**
- [ ] 테스트 파일 생성: `caret-src/__tests__/init-context-separation.test.ts`
- [ ] 5개 테스트 케이스 작성
- [ ] 테스트 실행 (모두 실패 예상)
...
```bash
# 이미 확인된 WebP 관련 코드
caret-src/utils/image-optimization.ts  # 최적화 유틸리티
...
```bash
# 1. 이미지 최적화 로직 확인
cat caret-src/utils/image-optimization.ts

# 2. 이미지 파일 직접 읽기 로직 확인
grep -r "readImage\|getImageContent" caret-src/ --include="*.ts"

# 3. 이미지 툴 스펙 확인
cat caret-src/core/prompts/system-prompt/tools/generate_image.ts

# 4. 이미지 처리 프로세스 확인
grep -A 20 "optimizeImageDataUrl" caret-src/utils/image-optimization.ts
```
...
```typescript
// caret-src/__tests__/image-tool-optimization.test.ts (신규)
...
```typescript
// caret-src/utils/image-optimization.ts (수정)
...
```typescript
// caret-src/core/task/tools/utils/image-reader.ts (신규)
...
```typescript
// caret-src/core/task/tools/handlers/GenerateImageToolHandler.ts (수정)
...
**Phase 0: RED**
- [ ] 테스트 파일 생성: `caret-src/__tests__/image-tool-optimization.test.ts`
- [ ] 5개 테스트 케이스 작성
- [ ] 테스트 실행 (모두 실패 예상)
...
| 문서 | 경로 |
|------|------|
| **Caret 개발 원칙** | `.agents/context/caret-rules.json` |
| **TDD 가이드** | `caret-docs/merging/tdd-testing-requirements.md` |
| **이미지 플로우** | `caret-docs/features/f14-image_flow.md` |
| **머징 가이드** | `caret-docs/merging/merge-standard-guide.md` |
