...
### **방식**
전체 upstream을 squash merge 후 Caret 기능 복원
...
| **Proto/타입 복원** | 0.5일 | Caret 타입 보호 |
| **Critical Files 복원** | 0.5-1일 | App.tsx, Providers.tsx 등 |
| **Caret 디렉토리 복원** | 0.5일 | caret-src, caret-docs 등 |
...
```bash
# 시나리오 1: 백업 누락
git reset --hard upstream/main  # caret-src 삭제!
# → 백업 없으면 Caret 기능 모두 소실

# 시나리오 2: 브랜딩 복원 누락
# Proto package "caret" → "cline"으로 변경됨
# → gRPC 서버 시작 실패

# 시나리오 3: Critical Files 누락
# App.tsx의 CaretI18nProvider 누락
...
| **충돌 해결** | 상대적으로 적음 | Caret 파일만 보호 | 🍒 Cherry-Pick |
| **테스트 범위** | 단위 테스트 가능 | 전체 테스트 필요 | 🍒 Cherry-Pick |
| **롤백 용이성** | 개별 기능 롤백 | 전체 롤백 | 🍒 Cherry-Pick |
| **Caret 독립성** | 완전 유지 | 부분 손실 후 복원 | 🍒 Cherry-Pick |
...
2. **난이도**
   - 프로세스가 단순
   - 이미 검증된 프로세스 (caret-docs/merging/merge-standard-guide.md)
...
cp -r caret-src/ caret-docs/ assets/ caret-scripts/ /tmp/caret-backup-$(date +%Y%m%d)/
...
# 5. 통합 커밋 생성
git add -A
git commit -m "Squash Merge: Cline v3.49.0+ → Caret (2026-01-14)
...
- Context Providers: CaretI18nProvider, CaretStateContextProvider
...
📚 Reference: caret-docs/merging/merge-standard-guide.md"

# 6. Baseline 태그 생성 (향후 rebase용)
git tag caret-squash-baseline-$(date +%Y%m%d)
```
...
cp /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/App.tsx \
   webview-ui/src/App.tsx

# 2. Providers.tsx (순서 확인)
cp /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/Providers.tsx \
   webview-ui/src/Providers.tsx

# 3. WelcomeView.tsx (언어/API/Persona 플로우)
cp /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/components/welcome/WelcomeView.tsx \
   webview-ui/src/components/welcome/

# 4. ChatView.tsx (Persona 아바타)
cp /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/components/chat/ChatView.tsx \
   webview-ui/src/components/chat/

# 5. SettingsView.tsx + sections/ (General/About 탭)
cp /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/components/settings/SettingsView.tsx \
   webview-ui/src/components/settings/
cp -r /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/components/settings/sections/ \
      webview-ui/src/components/settings/

# 6. ApiOptions.tsx (Provider 필터링)
cp /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/components/settings/ApiOptions.tsx \
   webview-ui/src/components/settings/

# 7. ClineRulesToggleModal.tsx (Persona 관리)
cp /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/components/cline-rules/ClineRulesToggleModal.tsx \
   webview-ui/src/components/cline-rules/

# 8. task-header/ (HomeHeader, Persona 이미지)
cp -r /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/components/chat/task-header/ \
      webview-ui/src/components/chat/

# 9. caret 디렉토리 전체 복사
cp -r /tmp/caret-backup-$(date +%Y%m%d)/webview-ui/src/caret/ \
...
1. **Chat Streaming UI 충돌**
   ...
   - 해결: Caret 브랜딩 적용, PersonaAvatar 통합

2. **Skills/Hooks 시스템 충돌**
   - 문제: `.cline/skills` → `.caret/skills` 필요
   - 해결: 경로 수정, Caret 프롬프트 시스템 통합
...
| 문서 | 경로 | 내용 |
|------|------|------|
| **Squash Merge 가이드** | `caret-docs/guides/upstream-merging.md` | Squash Merge 전략 |
| **머징 표준 가이드** | `caret-docs/merging/merge-standard-guide.md` | Phase 0-5 프로세스 |
| **머징 체크리스트** | `caret-docs/merging/merge-standard-guide.md#다음-머징-회차-체크리스트` | 상세 체크리스트 |
