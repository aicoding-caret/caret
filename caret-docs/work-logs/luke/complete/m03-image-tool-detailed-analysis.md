...
**의존성**: `caret-src/utils/image-optimization.ts`
...
- 이미지 ID 생성 (`caret-src/shared/images/image-id.ts`)
...
```bash
# 비전 모델 지원 확인
grep -r "vision\|Vision" caret-src/
# 결과: 없음
...
```typescript
// caret-src/utils/image-optimization.ts
...
import { ImageReader } from "@caret/core/task/tools/utils/image-reader"
...
```typescript
// caret-src/core/task/tools/utils/vision-analyzer.ts (신규)
...
```typescript
// caret-src/core/task/tools/utils/image-cache.ts (신규)
