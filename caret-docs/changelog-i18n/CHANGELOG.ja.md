...
> **注**: Caret v0.4.5はCline v3.49.0+からSkillsシステム、Hooks i18nなどをチェリーピックしています。
...
- **analyze_imageツール**: CaretアカウントのGeminiと連携した画像分析ツールを新しく追加しました。最大7500px制限適用、分析結果レポートガイドライン含む。
...
- **デフォルトプロバイダー**: 新規ユーザーのデフォルトプロバイダーがCaretに設定されます。
...
- **重複メッセージ表示**: 「Caret画像生成をリクエスト」メッセージが2回表示される問題を修正しました。
