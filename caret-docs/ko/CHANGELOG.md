...
> **참고**: Caret v0.4.0은 Cline v3.38.2를 기반으로 합니다.

### 🎉 Cline v3.38.2 업스트림 머징
- **머지 커밋**: `8723b386f` (머지 브랜치: `main_backup_20251128202033`)

### 추가됨
- **Cline v3.38.2 완전 통합**: 최신 모델 지원 및 핵심 개선 사항을 포함하여 Cline v3.38.2의 모든 기능을 통합했습니다. (Claude Opus 4.5 포함)
- **이중 계정 시스템 (Dual Account)**: Caret 모드(확장 기능)와 Cline 모드(기본 경험) 사이를 원활하게 전환할 수 있습니다.
- **Caret CLI (베타)**: 두 모드와 강화된 인증을 지원하는 새로운 통합 CLI 도구 `caret`을 출시했습니다. LiteLLM 지원이 포함됩니다.
- **향상된 프로바이더 설정**: LiteLLM 및 BizRouter 모델 자동 검색 및 실시간 상태 확인 기능을 제공합니다.
- **JSON 프롬프트 시스템**: JSON 파일을 통한 동적 시스템 프롬프트 구성을 지원합니다.
- **입력 히스토리**: 터미널과 유사한 입력 히스토리 탐색 기능을 영구 저장소와 함께 제공합니다.
- **단축키**: 작업 취소(Esc) 및 재개(Ctrl+Shift+R) 단축키가 추가되었습니다.

### 수정됨
- **터미널 멈춤**: Linux 시스템에서 쉘 통합 사용 시 터미널 명령이 멈추는 문제를 수정했습니다.
- **브랜딩**: 모든 UI 구성 요소 및 CLI 출력에서 Caret 브랜딩을 올바르게 복원했습니다.
