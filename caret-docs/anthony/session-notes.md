...
2. 로그인 완료 후 리다이렉트: `/auth?...` 콜백이 확장 측 `SharedUriHandler.handleUri`(`src/services/uri/SharedUriHandler.ts`)에서 처리된다. 쿼리의 `token/refreshToken/idToken/code`와 `provider`를 파싱해 `visibleWebview.controller.handleAuthCallback(token, provider || "caret")` 호출.
3. 컨트롤러 처리(`src/core/controller/index.ts:455`):
   - `provider !== "caret"`이면 `AuthService.handleAuthCallback` → 코드→토큰 교환, `_clineAuthInfo` 세팅, `_authenticated=true`, `sendAuthStatusUpdate`.
   - `provider === "caret"`이면 Caret 토큰 저장 후 API provider 기본값을 feature-config 기본 provider로 설정하고 상태 저장.
...
## 5) 인증 토큰 처리 흐름
- 저장/교환:
  - `/auth` 콜백 → `SharedUriHandler.handleUri` → `Controller.handleAuthCallback` → (비 caret) `AuthService.handleAuthCallback`가 `IAuthProvider.signIn`(여기서는 `ClineAuthProvider.handleAuthCallback`)을 통해 `accessToken/refreshToken/expiresAt/userInfo`를 받아 `_clineAuthInfo` 저장.
