...
이 문서는 Caret CLI (Command-Line Interface)의 개발, 빌드, 테스트 절차를 안내합니다. **`package.json`의 `scripts` 섹션이 모든 명령어의 유일한 진실 공급원(Single Source of Truth)입니다.**
...
Caret CLI는 Go 언어로 작성되었으며, 터미널 환경에서 Caret의 핵심 기능과 상호작용할 수 있는 강력한 도구입니다. VSCode 익스텐션과 gRPC를 통해 통신하며, 자동화된 작업, 계정 관리, 시스템 상태 확인 등의 기능을 제공합니다.
...
CLI 코드 작성 시, `Caret` 또는 `Cline`과 같은 브랜드명을 직접 하드코딩해서는 안 됩니다. B2B 브랜딩 요구사항을 지원하기 위해 반드시 아래의 공유 유틸리티 함수를 사용해야 합니다.
...
#### `scripts/caret-build-run.sh`

- **설명**: CLI 전체를 새로 빌드하고 실행하는 가장 일반적인 스크립트입니다.
- **실행 명령어**: `scripts/caret-build-run.sh [args...]`
- **주요 특징**:
...
#### `scripts/caret-run.sh`

- **설명**: 빌드 과정을 건너뛰고 이전에 빌드된 실행 파일을 즉시 실행합니다.
- **실행 명령어**: `scripts/caret-run.sh [args...]`
- **주요 특징**:
...
#### `scripts/caret-build-auth.sh`

- **설명**: `auth` 명령어 테스트에 특화된 **빌드 및 실행** 스크립트입니다.
- **실행 명령어**: `scripts/caret-build-auth.sh`
- **주요 특징**:
    - 기본으로 실행 중인 `caret-host/cline-host/cline-core`를 종료하고 `.caret/.cline` locks DB를 정리한 뒤 빌드를 시작합니다. 기존 인스턴스를 보존하려면 `CARET_SKIP_KILL=1`을 설정하세요.
    - `caret-build-run.sh`와 동일하게 전체 빌드 과정을 수행합니다.
    - 빌드 후 `auth -v --no-version-check` 명령어를 고정적으로 실행합니다.
- **사용 시기**: `auth` 관련 기능을 수정한 후, 전체 빌드를 포함한 통합 테스트를 수행할 때 사용합니다.

---

#### `scripts/caret-run-auth.sh`

- **설명**: `auth` 명령어 테스트에 특화된 **빠른 재실행** 스크립트입니다.
- **실행 명령어**: `scripts/caret-run-auth.sh`
- **주요 특징**:
    - 빌드 과정 없이 `cli/bin/caret auth -v --no-version-check` 명령어를 즉시 실행합니다.
...
- `caret-docs/development/caret-cli-auth-flow.md`

## 일반적인 개발 워크플로우

1.  **Go 코드 수정**: `cli/pkg/` 또는 `cli/cmd/` 내의 Go 소스 코드를 수정합니다.
2.  **gRPC 정의 수정 (필요시)**: 익스텐션과의 통신 인터페이스를 변경해야 할 경우, `proto/caret/` 내의 `.proto` 파일을 수정합니다.
