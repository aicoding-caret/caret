...
Fixes a VS Code activation failure when both Caret and Cline are installed:
`Activating extension 'saoudrizwan.claude-dev' failed: command 'cline.plusButtonClicked' already exists.`

- **Root cause**: Caret was registering the same global command IDs as Cline (e.g., `cline.plusButtonClicked`), and VS Code requires command IDs to be unique across extensions.
- **Fix**: namespace Caret commands under the extension id prefix, e.g. `caretive.caret.plusButtonClicked`.
- **Notes**:
  - This is a breaking change for any user-defined keybindings or automation referencing the old `cline.*` command IDs in Caret.
...
| **VS Code** | `package.json` | Renames `contributes.commands` + menus + keybindings from `cline.*` → `caretive.caret.*`. |
| **VS Code** | `src/registry.ts` | Uses `${publisher}.${name}` as command prefix to avoid Cline collisions. |
| **Dev** | `src/dev/commands/tasks.ts` | Renames dev command IDs to `caretive.caret.dev.*`. |
...
6. **Coexistence**  
   - Install both extensions (Caret + Cline) and confirm neither fails activation due to duplicate command IDs.
