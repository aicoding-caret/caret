...
The `read_document` tool enables LLM to read various document files **by path only** - a Caret-exclusive feature.
...
| PowerPoint | `.pptx` | Caret implementation | ZIP + XML parsing |
| Hangul (Modern) | `.hwpx` | Caret implementation | ZIP + XML parsing |
