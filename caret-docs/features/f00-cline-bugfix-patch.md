...
Caret와 Cline을 동시에 설치했을 때 아래 오류로 Cline 활성화가 실패할 수 있습니다:
`Activating extension 'saoudrizwan.claude-dev' failed: command 'cline.plusButtonClicked' already exists.`

- **원인**: Caret가 Cline과 동일한 전역 command ID(예: `cline.plusButtonClicked`)를 등록하고 있었고, VS Code는 command ID가 전역 유니크여야 함.
- **해결**: Caret command를 확장 ID prefix로 네임스페이스 분리 (예: `caretive.caret.plusButtonClicked`).
- **주의**:
  - Caret의 기존 `cline.*` command ID를 직접 참조하던 사용자 커스텀 키바인딩/자동화가 있다면 업데이트가 필요.
...
| **VS Code** | `package.json` | `contributes.commands` + menus + keybindings의 `cline.*` → `caretive.caret.*` 변경. |
| **VS Code** | `src/registry.ts` | command prefix를 `${publisher}.${name}`로 사용해 Cline과 충돌 방지. |
| **Dev** | `src/dev/commands/tasks.ts` | dev command ID를 `caretive.caret.dev.*`로 변경. |
...
6. **동시 설치(공존)**
   - Caret + Cline 동시 설치 후, 어느 쪽도 command ID 중복으로 활성화 실패하지 않는지 확인.
