...
> **注**: Caret v0.4.0 は Cline v3.38.2 をベースにしています。

### 🎉 Cline v3.38.2 アップストリームマージ
- **マージコミット**: `8723b386f` (マージブランチ: `main_backup_20251128202033`)

### 追加
- **Cline v3.38.2 完全統合**: 最新のモデルサポートとコア改善（Claude Opus 4.5を含む）を含め、Cline v3.38.2の全機能を統合しました。
- **デュアルアカウントシステム**: Caretモード（拡張機能）とClineモード（基本体験）をシームレスに切り替え可能。
- **Caret CLI (ベータ)**: 両モードと強化された認証をサポートする新しい統合CLIツール `caret` をリリースしました。LiteLLMサポートも含まれます。
- **プロバイダー設定強化**: LiteLLMおよびBizRouterモデルの自動取得とリアルタイムヘルスチェック機能を提供。
- **JSONプロンプトシステム**: JSONファイルによる動的システムプロンプト構成をサポート。
- **入力履歴**: 永続ストレージを備えたターミナル風の入力履歴ナビゲーションを提供。
- **ショートカット**: タスクのキャンセル（Esc）と再開（Ctrl+Shift+R）のショートカットを追加しました。

### 修正
- **ターミナルのハング**: Linuxシステムでシェル統合を使用する際にターミナルコマンドがハングする問題を修正しました。
- **ブランディング**: すべてのUIコンポーネントとCLI出力でCaretブランディングを正しく復元しました。

## [3.38.2]

### 追加
- Claude Opus 4.5 を追加（Anthropic / OpenRouter / Bedrock / Vertex、プロンプトキャッシュ料金対応）
